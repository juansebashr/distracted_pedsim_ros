   ...
   public:
    ...
    void Load(physics::WorldPtr _world, sdf::ElementPtr _sdf)
    {
        ...
        ros::SubscribeOptions so = ros::SubscribeOptions::create<pedsim_msgs::AgentStates>(
            "/pedsim_simulator/simulated_agents", 1, boost::bind(&ActorPosesPlugin::OnRosMsg, this, _1), ros::VoidPtr(),
            &rosQueue);
        ...
        // this->updateConnection_ = event::Events::ConnectWorldUpdateBegin(std::bind(&ActorPosesPlugin::OnUpdate,
        // this));
    }
    ...
    void OnRosMsg(const pedsim_msgs::AgentStatesConstPtr msg)
    {
        ...
        for (unsigned int mdl = 0; mdl < world_->ModelCount(); mdl++)
        {
            ...
            for (uint actor = 0; actor < msg->agent_states.size(); actor++)
            {
                ...
                {
                    //                            ROS_INFO_STREAM("actor_id: "<< std::to_string(
                    ...
                    gzb_pose.Rot().Set(
                        msg->agent_states[actor].pose.orientation.w, msg->agent_states[actor].pose.orientation.x,
                        msg->agent_states[actor].pose.orientation.y, msg->agent_states[actor].pose.orientation.z);

...
SceneServices::SceneServices()
{
    ...
    remove_all_peds_service_ =
        nh_.advertiseService("pedsim_simulator/remove_all_peds", &SceneServices::removeAllPeds, this);
    reset_peds_service_ = nh_.advertiseService("pedsim_simulator/reset_all_peds", &SceneServices::resetPeds, this);
    add_obstacle_service_ =
        nh_.advertiseService("pedsim_simulator/add_obstacle", &SceneServices::addStaticObstacles, this);
    move_peds_service_ =
        nh_.advertiseService("pedsim_simulator/move_peds", &SceneServices::moveAgentClustersInPedsim, this);
    respawn_interactive_obstacles_service_ = nh_.advertiseService("pedsim_simulator/respawn_interactive_obstacles",
                                                                  &SceneServices::respawnInteractiveObstacles, this);
    spawn_interactive_obstacles_service_ = nh_.advertiseService("pedsim_simulator/spawn_interactive_obstacles",
                                                                &SceneServices::spawnInteractiveObstacles, this);
    remove_all_interactive_obstacles_service_ = nh_.advertiseService(
        "pedsim_simulator/remove_all_interactive_obstacles", &SceneServices::removeAllInteractiveObstacles, this);

    // Check if flatland is the chosen simulation environment
    std::string environment;
    nh_.param<std::string>("/environment", environment, "flatland");
    if (environment == "gazebo") env_is_flatland = false;
    if (env_is_flatland)
    {
...
bool SceneServices::respawnPeds(pedsim_srvs::SpawnPeds::Request& request, pedsim_srvs::SpawnPeds::Response& response)
{
...
bool SceneServices::respawnInteractiveObstacles(pedsim_srvs::SpawnInteractiveObstacles::Request& request,
                                                pedsim_srvs::SpawnInteractiveObstacles::Response& response)
{
    // remove old
...
bool SceneServices::spawnInteractiveObstacles(pedsim_srvs::SpawnInteractiveObstacles::Request& request,
                                              pedsim_srvs::SpawnInteractiveObstacles::Response& response)
{
...
void SceneServices::removeAllInteractiveObstaclesFromFlatland() { removeModelsInFlatland(static_obstacle_names_); }

bool SceneServices::removeAllInteractiveObstacles(std_srvs::Trigger::Request& request,
...
void SceneServices::addAgentClusterToPedsim(pedsim_msgs::Ped ped, std::vector<int> ids)
{
    ...
    for (int i = 0; i < ped.number_of_peds; i++)
    {
        ...
        if (a->getType() == Ped::Tagent::AgentType::ADULT || a->getType() == Ped::Tagent::AgentType::ELDER ||
...
bool SceneServices::removeModelsInFlatland(std::vector<std::string> model_names)
{
    ...
    {
        ROS_ERROR("Failed to delete all %d models. Maybe a few were deleted. Flatland response: %s",
                  int(msg.request.name.size()), msg.response.message.c_str());
        return false;
    }
...
// bool SceneServices::respawnModelsInFlatland(std::vector<std::string> old_model_names,
// std::vector<flatland_msgs::Model> new_models) {
...
std::vector<Obstacle*> SceneServices::getWallsFromFlatlandModel(pedsim_msgs::InteractiveObstacle obstacle, double yaw)
{
    ...
    if (type.as<std::string>() == "polygon")
    {
        ...
        for (int i = 0; i < (int)points.size() - 1; i++)
        {
            ...
            Obstacle* wall = new Obstacle(pos.x + start.x, pos.y + start.y, pos.x + end.x, pos.y + end.y);
        ...
        Obstacle* closing_wall = new Obstacle(pos.x + first.x, pos.y + first.y, pos.x + last.x, pos.y + last.y);
